...
#define PAN_MAX  4096   //Maximum Servo Position
...
#define MAPPED_PAN_MAX 4096 //Maximum Physical Position on a scale from 0-4096.  Mapped value is the same as the PAN value for MX servos
#define TILT_MIN  1500      //Minimum Tilt Servo Position
#define TILT_MAX  3000   //Maximum Tile Servo Position The 
#define MAPPED_TILT_MIN 1500  //Minimum Physical Position on a scale from 0-2048. Mapped value is the same as the TILT value for MX servos
...
#define DEFAULT_SPEED1 32
#define DEFAULT_SPEED2 32
...
int intervalMoving = 33;        // interval at which to request moving data from the servo
...
void setup()
{
  ...
  SetPosition(1,2828);     //set the position of servo # 1 to its starting/min position
  delay(33);                   //delay before next DYNAMIXEL command
  
  SetPosition(2,1881); //set the position of servo # 2 to  its defualt position. If the servo is not present, nothing will happen
  delay(33);                    //delay before next DYNAMIXEL command
...
void llWriteAndWait(char myAddress, char myValue){
  ...
  uint8_t nackack = 1000; // Setup variable to hold ACK/NACK resopnses     
