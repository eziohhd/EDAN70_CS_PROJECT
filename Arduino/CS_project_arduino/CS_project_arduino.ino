...
boolean startFlag = false;      //true if the servo is ok to scan
boolean scanActive = false;     //false if the servo is not activley scanning, true if it is activley scanning
boolean systemActive = false;     //false if the system is not ready, true if it is ready for calibration and experiment
...
{ 
  if(startFlag) {
    if(tiltCurrentPosition == TILT_MAX)
    {
      ...
      startFlag = false;
      systemActive = true; //done with scanning, ready to calibrate the system
    }
    else
    {
      scanActive = true;
